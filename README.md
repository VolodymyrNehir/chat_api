...
У дереві є згенерований скаффолдом `test/app.e2e-spec.ts` (перевіряє лише
`/health`), але він не підпадає під `testRegex` команди `npm test` і не
запускається `make test` — тобто присутній у репозиторії, але не є частиною
тестового прогону.
